...
def genScript(tol,bmk,comp,nod,app):
	jobName = tol+"."+bmk+"."+app+"."+comp+"."+nod
	print jobName
	s = s_stampede_config(jobName,nod,"00:15:00")
	s = s + s_common(jobName)
	for tl in [x for x in listOfTools if x.tool == tol]:
		for bm in [x for x in listOfBenchmarks if x.benchmark == bmk and x.node == nod and x.compiler == comp and x.app == app]:
			s = s + s_body(tl,bm)
...
if __name__ == '__main__':
	...
	for i in cc:
		for j in nasApps:
			genScript("npinMain","nas","mpicc",i,j)

			genScript("npinAll","nas","mpicc",i,j)
