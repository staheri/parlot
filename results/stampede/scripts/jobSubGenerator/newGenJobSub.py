...
def s_common(jobName):
	...
	s = s + "JOB="+jobName+"\n"
	s = s + "COMMON_DIR=$SCRATCH/main/experiments\n"
...
def s_body(tl,bm):
	...
	#		s = s + "cp " + tc + " $WORK_DIR\n"
		...
		for nf in tl.needed_files:
			s = s + "rm "+ext(nf)+" ; \n"
		if not ("npin" in tl.tool or "wpin" in tl.tool or "hpin" in tl.tool or "dpin" in tl.tool or ( tl.tool == "orig" and bm.compiler == "mpicc" )): 
...
def genScript(tol,bmk,comp,nod,app,flag):
	...
	print jobName
	if "wpin" in tol:
		s = s_stampede_config(jobName,nod,"10:00:00")
	else:
		s = s_stampede_config(jobName,nod,"01:00:00")
...
def genExecPath(common,flags,comp,top,app):
	s = common+flags + "/"
	if comp == "mpicc":
		s = s + "orig-C-"+top+"/"+app+"."+top
	else:
		s = s + comp+"-C-"+top+"/"+app+"."+top
	return s
...
#				genScript("orig","nas","scorep",i,j,k)
				genScript(ttl,"nas","mpicc",i,j,k)
#				genScript("pinMain","nas","mpicc",i,j,k)
#				genScript("pinAll","nas","mpicc",i,j,k)
#				genScript("dpinMain","nas","mpicc",i,j,k)
#				genScript("dpinAll","nas","mpicc",i,j,k)
