...
def s_common(jobName):
	s = "RUN_NAME="+runName+";\n"
	s = s + "EXPERIMENT="+exp_name+";\n"
	s = s + "JOB="+jobName+";\n"
	s = s + "TRACE_DIR=$WORKSPACE/$RUN_NAME/$EXPERIMENT/traces;\n"
	s = s + "APPOUT_DIR=$WORKSPACE/$RUN_NAME/$EXPERIMENT/app_out;\n"
	s = s + "EXEC_DIR=$WORKSPACE/$RUN_NAME/$EXPERIMENT/exec_dir;\n"
...
def s_body(tl,bm):
	...
	for i in range (1,numberOfRuns+1):
		...
		if (tl.tool == "hpinMain" or tl.tool == "hpinAll" ):
			s = s + "mv $LOCAL_STORAGE/Hd* $TRACE_DIR/$JOB."+`i`+"/ ;\n\n"
		elif tl.tool=="hcallgrind":
			s = s + "mv $LOCAL_STORAGE/callgrind.* $TRACE_DIR/$JOB."+`i`+"/ ;\n\n"
		elif "wpin" in tl.tool:
			s = s + "rm $TEMP_DIR/* "
		elif tl.tool=="pinMain" or tl.tool=="pinAll" or tl.tool=="callgrind":
			s = s + "mv $TEMP_DIR/* $TRACE_DIR/$JOB."+`i`+"/ ;\n\n"
		s = s +"\ncd $EXEC_DIR ; \n"
		s = s +"\nrm -rf $TEMP_DIR ; \n"
		i = i + 1
	s=s+"\n\n####################### Runs End ###########################\n\n"
	...
	for tl in [x for x in listOfTools if x.tool == tol]:
		for bm in listOfBenchmarks:
			if bm.benchmark == bmk and bm.node == nod and bm.compiler == comp and bm.app == app and bm.flag == flag and bm.psize == psize:
				s = s + s_body(tl,bm)
	sub = open (pathToJobSub+"/"+runName+"/"+exp_name+"/"+jobName+".slurm","w")
	sub.write(s)
	sub.close()
...
if __name__ == '__main__':
        ...
        exp_name = sys.argv[4]
        ttl =  sys.argv[3]
        numberOfRuns=int(sys.argv[5])
        psize=sys.argv[6]
        node=sys.argv[7]
        flag=sys.argv[8]
        runName = sys.argv[9]
	process = subprocess.Popen(["mkdir -p "+pathToJobSub+"/"+runName+"/"+exp_name], stdout=subprocess.PIPE,shell=True)
	si, err = process.communicate()
	#process = subprocess.Popen(["cp $SCRATCH/submitJobs.py "+pathToJobSub+"/"+runName+"/"+exp_name], stdout=subprocess.PIPE,shell=True)
	#si, err = process.communicate()
	server = "psc"
	bench = "nas"
	compiler = "mpicc"
	...
	readConfig(CONFIG_FILE,server)
		...
		print key
                if "wpin" in ttl or "callgrind" in ttl:
                        if node == "1" and (psize == "C" or psize == "D"):
                                genScript(key,"02:00:00")
                        else:
                                genScript(key,"00:15:00")
                        
                else:
                        genScript(key,"00:10:00")
