...
class integrate_objects():
	...
	def avg(self,list):
		...
		if len(list) != 0:
			...
			return 0.0
	def stat(self,base):
		self.stat_med["sd"] = (self.rt_med * 1.0 )/(base.rt_med * 1.0) if base.rt_med != 0 else 0.0
		self.stat_avg["sd"] = (self.rt_avg * 1.0 )/(base.rt_avg * 1.0) if base.rt_avg != 0 else 0.0
		cores = int(self.key.split(".")[-1])*16
		self.stat_avg["bw"] = ( (self.sz_avg * 1.0) / cores ) / self.rt_avg if self.rt_avg != 0 else 0.0
		self.stat_med["bw"] = ( (self.sz_med * 1.0) / cores ) / self.rt_med if self.rt_med != 0 else 0.0
